...
//@desc     Get posts by id //so that anyone can read posts
//@access   Public
    ...
    .then(profile => {
      Post.findById(req.params.id)
        .then(post => {
          ...
          .length === 0)//equals 0 means they're not there 
          {
...
router.delete('/comment/:id/:comment_id', passport.authenticate('jwt', {session: false}), (req, res) => {
 ...
 Post.findById(req.params.id)
  .then(post => {
    ...
    if(post.comments.filter(comment => comment._id.toString() === req.params.comment_id). length === 0) { //if this is true it  means the comment doesn't exist
